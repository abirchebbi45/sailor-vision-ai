...
class DetectionRecorder(QObject):
    ...
    def __init__(self, ros_node, ros_bridge=None):
        """Initialize the detection recorder."""
        super().__init__()
        self.ros_node = ros_node  # Correctly assign the ROS node
        self.ros_bridge = ros_bridge
        
        # Initialize ros_watchdog as None - it will be set by app.py
        self.ros_watchdog = None
        ...
        self.output_dir = os.path.join("recordings", "detections")
        ...
        self.alert_subscriber = self.ros_node.create_subscription(
        ...
        self.bridge = CvBridge()  # <-- Add this line to fix the error
    ...
    def _start_recording(self, topic):
        ...
        if topic not in self.frame_buffer or len(self.frame_buffer[topic]) == 0:
            logger.warning(f"No pre-recorded frames available for {topic}, skipping recording.")
            return
